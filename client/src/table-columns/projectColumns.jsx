...
export const createProjectColumns = ({ onEdit }) => [
  {
    header: 'Project',
    accessorKey: 'name',
    sortable: true,
    ...
    cell: (cellProps) => {
      ...
      const name = item?.name || item?.mpjt_name || '—'
      const id = item?.id || item?.mpjt_id
  ...
  },
  {
    ...
    cell: (cellProps) => {
      const item = getItem(cellProps)
      const status = String(item?.status || item?.mpjt_status || 'ACTIVE').toUpperCase()
    ...
    cell: (cellProps) => {
      ...
      return (
        <div className="flex items-center justify-center gap-1.5 text-slate-700 text-xs font-semibold whitespace-nowrap w-full">
          ...
          <span>{formatDate(item?.createdAt || item?.mpjt_createdAt)}</span>
        </div>
      )
    },
  },
