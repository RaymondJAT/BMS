...
export default function EditDepartmentModal({
  ...
  useEffect(() => {
    if (department) {
      setStatus(department.md_status || department.status || 'ACTIVE')
    }
    setFormError(null)
  }, [department, isOpen])
  ...
  const handleSubmit = async (e) => {
    ...
    const result = await onUpdateStatus({
      id: department.md_id || department.id,
      status,
    })
    ...
    if (result?.success || result?.status === 200) {
      ...
      setFormError(result?.message || 'Failed to update department status.')
    }
  }
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title="Edit Department Status"
      subtitle={`Code: ${department.md_code || department.code || 'N/A'}`}
      maxWidth="max-w-md"
    >
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <input
            type="text"
            readOnly
            value={department.md_name || department.name || ''}
            className="w-full px-2.5 py-1.5 bg-slate-100 border border-slate-200 rounded-lg text-xs font-semibold text-slate-600 focus:outline-none cursor-not-allowed"
          />
        </div>
